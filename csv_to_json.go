...
func csvToJSON(context *cli.Context) error {
	...
	if infile := context.String(inputFileFlagName); len(infile) < 1 {
	...
	w := &writer{os.Stdout}
	if outfile := context.String(outputFileFlagName); len(outfile) > 0 {
	...
	reader.Comma = rune(context.String(delimiterFlagName)[0])
